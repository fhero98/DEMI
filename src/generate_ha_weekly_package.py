...
# =========================================================
# Generate Weekly Home Assistant Package
# =========================================================
def generate_home_assistant_weekly_summary_package() -> Path:
    vehicles = load_vehicle_ids_from_excel()

    output_dir = current_file_dir / "ha_generated"
    output_dir.mkdir(parents=True, exist_ok=True)

    output_path = output_dir / "ev_weekly_summary.yaml"

    lines = []
    lines.append("# Generated automatically from the EV Excel file")
    lines.append("# Copy this file to: /config/packages/ev_weekly_summary.yaml")
    lines.append("")
    ...
    for vehicle in vehicles:
        ...
        lines.append(f"  ev_weekly_completed_{slug}:")
        lines.append(f"    name: EV Weekly Completed {raw_id}")
        lines.append("    initial: 0")
        lines.append("    step: 1")
        lines.append("")

        lines.append(f"  ev_weekly_failed_{slug}:")
        lines.append(f"    name: EV Weekly Failed {raw_id}")
        lines.append("    initial: 0")
        lines.append("    step: 1")
        lines.append("")
    ...
    for vehicle in vehicles:
        ...
        lines.append(f"  ev_weekly_energy_{slug}:")
        lines.append(f"    name: EV Weekly Energy {raw_id}")
        lines.append("    min: 0")
        ...
        lines.append(f"  ev_weekly_duration_{slug}:")
        lines.append(f"    name: EV Weekly Duration {raw_id}")
        lines.append("    min: 0")
        lines.append("    max: 100000")
        lines.append("    step: 1")
        ...
        lines.append(f"  ev_weekly_max_power_{slug}:")
        lines.append(f"    name: EV Weekly Max Power {raw_id}")
        lines.append("    min: 0")
    ...
    # =====================================================
    lines.append("group:")
    lines.append("  ev_weekly_vehicle_completed_counters:")
    lines.append("    name: EV Weekly Vehicle Completed Counters")
    lines.append("    entities:")

    for vehicle in vehicles:
        lines.append(f"      - counter.ev_weekly_completed_{vehicle['slug']}")
    ...
    lines.append("  - alias: EV Update Weekly Analytics Per Vehicle")
    ...
    lines.append("      completed_counter: \"counter.ev_weekly_completed_{{ vehicle_slug }}\"")
    lines.append("      failed_counter: \"counter.ev_weekly_failed_{{ vehicle_slug }}\"")
    lines.append("      energy_entity: \"input_number.ev_weekly_energy_{{ vehicle_slug }}\"")
    lines.append("      duration_entity: \"input_number.ev_weekly_duration_{{ vehicle_slug }}\"")
    lines.append("      max_power_entity: \"input_number.ev_weekly_max_power_{{ vehicle_slug }}\"")
    ...
    # Weekly notification
    lines.append("  - alias: EV Weekly Charging Summary For All Vehicles")
    lines.append("    trigger:")
    lines.append("      - platform: time")
    lines.append("        at: \"20:00:00\"")
    lines.append("    condition:")
    lines.append("      - condition: time")
    lines.append("        weekday:")
    lines.append("          - sun")
    lines.append("    action:")
    lines.append("      - repeat:")
    lines.append("          for_each: \"{{ expand('group.ev_weekly_vehicle_completed_counters') | map(attribute='entity_id') | list }}\"")
    lines.append("          sequence:")
    lines.append("            - variables:")
    lines.append("                slug: \"{{ repeat.item | replace('counter.ev_weekly_completed_', '') }}\"")
    lines.append("                vehicle_name: \"{{ state_attr(repeat.item, 'friendly_name') | replace('EV Weekly Completed ', '') }}\"")
    lines.append("                completed: \"{{ states('counter.ev_weekly_completed_' ~ slug) }}\"")
    lines.append("                failed: \"{{ states('counter.ev_weekly_failed_' ~ slug) }}\"")
    lines.append("                energy: \"{{ states('input_number.ev_weekly_energy_' ~ slug) }}\"")
    lines.append("                duration: \"{{ states('input_number.ev_weekly_duration_' ~ slug) }}\"")
    lines.append("                max_power: \"{{ states('input_number.ev_weekly_max_power_' ~ slug) }}\"")
    lines.append("")
    lines.append("            - service: notify.mobile_app_fotis_device")
    lines.append("              data:")
    lines.append("                title: \"EV Weekly Summary {{ vehicle_name }}\"")
    lines.append("                message: >")
    lines.append("                  Completed sessions this week: {{ completed }}")
    lines.append("                  Failed sessions this week: {{ failed }}")
    lines.append("                  Total energy this week: {{ energy }} kWh")
    lines.append("                  Total duration this week: {{ duration }} min")
    lines.append("                  Max power this week: {{ max_power }} kW")
    lines.append("                data:")
    lines.append("                  tag: \"weekly_summary_{{ slug }}\"")
    lines.append("                  group: \"weekly_summary_{{ slug }}\"")
    lines.append("                  channel: \"EV Weekly Summary\"")
    lines.append("")
    lines.append("            - delay:")
    lines.append("                seconds: 2")
    lines.append("")

    # Reset weekly helpers
    lines.append("      - repeat:")
    lines.append("          for_each: \"{{ expand('group.ev_weekly_vehicle_completed_counters') | map(attribute='entity_id') | list }}\"")
    lines.append("          sequence:")
    lines.append("            - variables:")
    ...
    lines.append("              data:")
    lines.append("                entity_id: \"{{ 'counter.ev_weekly_completed_' ~ slug }}\"")
    lines.append("")
    lines.append("            - service: counter.reset")
    lines.append("              data:")
    lines.append("                entity_id: \"{{ 'counter.ev_weekly_failed_' ~ slug }}\"")
    lines.append("")
    lines.append("            - service: input_number.set_value")
    lines.append("              data:")
    lines.append("                entity_id: \"{{ 'input_number.ev_weekly_energy_' ~ slug }}\"")
    lines.append("                value: 0")
    lines.append("")
    lines.append("            - service: input_number.set_value")
    lines.append("              data:")
    lines.append("                entity_id: \"{{ 'input_number.ev_weekly_duration_' ~ slug }}\"")
    lines.append("                value: 0")
    lines.append("")
    lines.append("            - service: input_number.set_value")
    lines.append("              data:")
    lines.append("                entity_id: \"{{ 'input_number.ev_weekly_max_power_' ~ slug }}\"")
    lines.append("                value: 0")
    ...
    print(f"Generated Home Assistant weekly package: {output_path}")
...
if __name__ == "__main__":
    generate_home_assistant_weekly_summary_package()